...
\begin{eqnarray}
    ...
    % &.........\\
    % &(t+1) P(S_{t+1} \leq \Theta | \mathbb{F}_{t}) - B (1 - P(S_{t+1} \leq \Theta | \mathbb{F}_{t}))= \\ 
    % &(t+1) P(S_{t}+Z_{t+1} \leq \Theta | \mathbb{F}_{t}) - B (1 - P(S_{t}+Z_{t+1} \leq \Theta | \mathbb{F}_{t}))=\\
    % &(t+1) P(Z_{t+1} \leq \Theta-S_{t} | \mathbb{F}_{t}) - B (1 - P(Z_{t+1} \leq \Theta-S_{t+1} | \mathbb{F}_{t}))=\\
    % &(t+1) F_{Z}(\Theta-S_{t}) - B (1 - F_{Z}(\Theta-S_{t}))
\end{eqnarray}

% Since we aim to not exceed the specified threshold $\Theta$, then the result of the reward function has to always be a positive value $E[V_t] > 0$. From this we can derive the probability of the absolute error sum not exceeding the threshold.
% \begin{multline}
%     E[V_t] = (t + B) \cdot P(S_t \leq \Theta) - B > 0\\
%     =>  P(S_t \leq \Theta) > \frac{B}{t+B}
% \end{multline}
...
\multicolumn{1}{|c|}{\cellcolor[HTML]{c1fbc0}Model type} & \multicolumn{1}{c|}{\cellcolor[HTML]{c1fbc0}\begin{tabular}[c]{@{}c@{}}High quality \\ predictions\end{tabular}} & \multicolumn{1}{c|}{\cellcolor[HTML]{c1fbc0}\begin{tabular}[c]{@{}c@{}}Low quality \\ predictions\end{tabular}} \\ \hline
