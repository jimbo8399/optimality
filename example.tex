...
\While{$True$}
    ...
    \State $absError \gets \mid mse - msePrime \mid$
    \If{cdf($rewardDist, (\Theta-absErrorSum$)) <\\\hspace{4.4cm} $(t+B)/(t+1+B)$}
...
% $\bullet$ \emph{CuSum policy:}
% $\bullet$ \emph{Random policy:}
% $\bullet$ \emph{OP policy:}
% $\bullet$ \emph{Median policy:}
...
%chosen B is 3 with highest mean waiting time
\begin{figure}[h]
    \centering
