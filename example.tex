...
\begin{abstract}
This paper explores the idea of deciding when to update statistical learning models in Intelligent Edge Devices given the underlying change in the data distribution. The model update schedule takes into consideration the optimal time for minimizing the network overhead while minimizing the prediction accuracy of the models. The paper presents a comparison between a proposed model updating approach and other 4 different update delaying policies, which evaluates the performances and discusses the strengths and weaknesses of the proposed policy.
\end{abstract}
...
Given that we aim is to not pass the specified threshold $\Theta$, then the result of the reward function has to always be a positive number $E[V_t] > 0$. This leads to deriving the probability of the absolute error sum not exceeding the threshold.
\begin{multline*}
